      ...
      <div v-if="currentPage === 'home'" :key="'home'" class="relative">
        ...
        <main class="flex flex-col items-center justify-center px-6 py-8 min-h-[calc(100vh-72px)]">
          ...
          <div v-else-if="isRolling" class="text-center w-full max-w-sm">
            <div class="slot-machine-container h-32 overflow-hidden relative">
              <div 
                v-for="(place, index) in visiblePlaces" 
                :key="index"
                class="absolute w-full text-3xl font-bold text-primary py-4 transition-all duration-100"
                :style="{ top: `${(index) * 100}%` }"
              >
                {{ place.name }}
              </div>
            </div>
            <p class="text-text-lighter mt-8 animate-pulse">正在帮你选择中...</p>
          </div>
...
const startRandom = () => {
  ...
  const animate = (timestamp) => {
    ...
    if (elapsed < duration) {
      ...
      visiblePlaces.value = [
        filtered[(randomIndex - 1 + filtered.length) % filtered.length],
        filtered[randomIndex],
        filtered[(randomIndex + 1) % filtered.length]
      ]
